...
On SDK 56, `@expo/ui` works in Expo Go, so `bunx expo start` runs it directly — no custom build required. On older SDKs, build a dev client first (`bunx expo run:ios` / `bunx expo run:android`).
