...
| **chekr** | `.cursor/skills/chekr/` | Chekr violations, audits, cache/prune, writing checks, parallel dedup remediation |
| **unit-agent** | `.cursor/skills/unit-agent/` | Orchestrating Junie subtasks via CLI (`--plan`, `--brave`, `--goal`, etc.) |
| **powershell-master** | `.cursor/skills/powershell-master/` | Advanced PowerShell scripting, CI/CD automation, cross-platform modules |
| **terminal-ops** | `.cursor/skills/terminal-ops/` | Evidence-first repository execution, command runs, verified fixes |
| **package-audit-and-cleanup** | `.cursor/skills/package-audit-and-cleanup/` | Auditing `package.json`, removing unused/redundant deps in monorepo |
| **backend-db-debugging** | `.cursor/skills/backend-db-debugging/` | Debugging backend services, DB schema inspection, migrations, and `.env` loading in scratch scripts |
...
Task arrives
    ...
    │
    ├── Chekr / check-violations / violations.json → chekr
    │
    ├── PowerShell scripting / automation → powershell-master
    │
    ├── CLI execution / evidence-first → terminal-ops
    │
    ├── Package audit / cleanup ──────────→ package-audit-and-cleanup
    │
...
| Chekr / check-violations / violations.json | `chekr` |
| Orchestrate subtasks, CLI execution | `unit-agent` |
| PowerShell / Scripting / Automation | `powershell-master` |
| CLI ops / Evidence / Execution | `terminal-ops` |
| Package Audit / Unused Deps | `package-audit-and-cleanup` |
| Backend & DB Debugging / Scratch Scripts | `backend-db-debugging` |
| `/clean-up` step 1 (violations) | `chekr` + `code-review-and-quality` |
...
| Chekr / architectural checks | `chekr` |
| Orchestrate subtasks / CLI | `unit-agent` |
| PowerShell expertise | `powershell-master` |
| Evidence-first CLI ops | `terminal-ops` |
| Package Audit & Cleanup | `package-audit-and-cleanup` |
| Backend & DB Debugging | `backend-db-debugging` |
