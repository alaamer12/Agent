...
1. **Detect target architecture & packaging**:
   - **Archives**: ZIP, JAR, APK, TAR, wheel (`inspect_binary.py <file> -p "<pattern>"`).
...
1. **Bytecode & Constant Pools**:
   - In JVM `.class` files, extract UTF-8 string tables and constants:
     ```bash
     python C:\Users\amrmu\.junie\skills\binary-reverse-engineering\scripts\inspect_binary.py <jar_or_class> -e "<ClassName>.class"
     ```
...
3. **Native Binaries**:
   - Extract ASCII and UTF-16LE strings: `python inspect_binary.py <file.exe> -s`.
...
1. **Reconstruct API Endpoints & Routes**:
   - Extract URL literals and route patterns:
     ```bash
     python C:\Users\amrmu\.junie\skills\binary-reverse-engineering\scripts\inspect_binary.py <target_path> -u
     ```
...
2. **Reconstruct Payloads & Authentication**:
   ...
   - Decode JWT tokens for inspection of claims and scopes:
     ```bash
     python C:\Users\amrmu\.junie\skills\binary-reverse-engineering\scripts\inspect_binary.py dummy -j "<jwt_token>"
     ```
...
- **`references/cli-recipes-and-cheatsheet.md`**: Complete command-line cookbook of battle-tested one-liners for PowerShell, Bash, and Python `-c` (in-memory archive exploration, constant-pool string dumps, URL carving, port snooping, and live endpoint probes).
...
- **`scripts/inspect_binary.py`**: Zero-dependency cross-platform utility to inspect archives (ZIP/JAR/APK/TAR), scan directory trees, parse JVM constant pools, extract binary strings, carve URLs/endpoints, and decode JWT payloads.
